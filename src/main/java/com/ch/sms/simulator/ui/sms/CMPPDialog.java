...
public class CMPPDialog extends Dialog {

	protected CMPPConfigure configure;
	 ...
	 * @param style
	 */
	public CMPPDialog(Shell parent, int style) {
		super(parent, style);
		setText("SWT Dialog");

	}

	public CMPPDialog(Shell parent, int style, CMPPConfigure configure) {
		super(parent, style);
		setText("SWT Dialog");
	...
	private void init() {
		if (configure != null) {
			...
			String[] items = this.comboVersion.getItems();
			for (int i = 0; i < items.length; i++) {
				SmsVersion version = (SmsVersion) this.comboVersion.getData(items[i]);
				if (version == configure.getVersion()) {
					this.comboVersion.select(i);
					break;
				}
			}
	...
	private void createContents() {
		...
		shlCmpp.setText("CMPP");
		shlCmpp.setSize(1024, 600);

		shlCmpp.setImage(SWTResourceManager.getImage(CMPPDialog.class, "/images/sms.jfif"));
		...
		label_3.setBounds(23, 111, 30, 24);
		label_3.setText("版本");

		comboVersion = new Combo(shlCmpp, SWT.SINGLE | SWT.BORDER | SWT.READ_ONLY);
		comboVersion.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		comboVersion.setBounds(130, 110, 167, 24);
		comboVersion.add(SmsVersion.CMPP20.getName());
		comboVersion.add(SmsVersion.CMPP30.getName());
		comboVersion.setData(SmsVersion.CMPP30.getName(), SmsVersion.CMPP30);
		comboVersion.setData(SmsVersion.CMPP20.getName(), SmsVersion.CMPP20);
	...
	protected boolean validateInput() {
		...
		if(this.comboVersion.getSelectionIndex()<0) {
			MessageBox box=new  MessageBox(shlCmpp,SWT.OK | SWT.ICON_ERROR | SWT.APPLICATION_MODAL);
			box.setText("错误");
			box.setMessage("请选择版本号");
			box.open();
			this.comboVersion.setFocus();
			return false;
		}
	...
	protected void fillConfigure(CMPPConfigure configure2) {
		...
		configure.setVersion((SmsVersion) this.comboVersion.getData(this.comboVersion.getText()));
