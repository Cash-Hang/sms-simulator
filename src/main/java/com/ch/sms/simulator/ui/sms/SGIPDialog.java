...
public class SGIPDialog extends Dialog {

	protected SGIPConfigure configure;
	...
	private Text textClientHost;
	private Label label_3;
	private Text textClientPort;
	private Label label_4;
	private Text textClientAccount;
	private Label label_5;
	private Text textClientPassword;
	 ...
	 * @param style
	 */
	public SGIPDialog(Shell parent, int style) {
		super(parent, style);
		setText("SWT Dialog");
	}
	...
	private void init() {
		if (configure != null) {
			...
			this.textClientHost.setText(configure.getClientHost()!=null?configure.getClientHost():StringUtils.EMPTY);
			this.textClientPort.setText(String.valueOf(configure.getClientPort()));
			this.textClientAccount.setText(configure.getClientAccount()!=null?configure.getClientAccount():StringUtils.EMPTY);
			this.textClientPassword.setText(configure.getClientPassword()!=null?configure.getClientPassword():StringUtils.EMPTY);
		}
	...
	private void createContents() {
		...
		shlCmpp.setText("SGIP");
		shlCmpp.setSize(1024, 600);
		shlCmpp.setImage(SWTResourceManager.getImage(SGIPDialog.class, "/images/sms.jfif"));
		...
		textReport.setVisible(false);
		textReport.addVerifyListener(new VerifyListener() {
			public void verifyText(VerifyEvent e) {
				TextVerifyUtils.numberTextVerify(e);
			}
		});
		
		lblNewLabel_2 = new Label(shlCmpp, SWT.NONE);
		lblNewLabel_2.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1));
		lblNewLabel_2.setText("客户端主机");
		
		textClientHost = new Text(shlCmpp, SWT.BORDER);
		textClientHost.setText("0.0.0.0");
		textClientHost.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		
		label_3 = new Label(shlCmpp, SWT.NONE);
		label_3.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		label_3.setText("客户端端口号");
		
		textClientPort = new Text(shlCmpp, SWT.BORDER);
		textClientPort.addVerifyListener(new VerifyListener() {
			public void verifyText(VerifyEvent e) {
				TextVerifyUtils.numberTextVerify(e);
			}
		});
		textClientPort.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		
		label_4 = new Label(shlCmpp, SWT.NONE);
		label_4.setText("客户端账号");
		
		textClientAccount = new Text(shlCmpp, SWT.BORDER);
		textClientAccount.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		
		label_5 = new Label(shlCmpp, SWT.NONE);
		label_5.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		label_5.setText("客户端密码");
		
		textClientPassword = new Text(shlCmpp, SWT.BORDER);
		textClientPassword.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
	...
	protected void fillConfigure(SGIPConfigure configure2) {
		...
		configure.setClientHost(this.textClientHost.getText());
		configure.setClientPort(Integer.parseInt(this.textClientPort.getText()));
		
		configure.setClientAccount(this.textClientAccount.getText());
		configure.setClientPassword(this.textClientPassword.getText());
	...
	protected boolean validateInput() {
		...
		String clientAccount=this.textClientAccount.getText();
		if(StringUtils.isBlank(clientAccount)) {
			MessageBox box=new  MessageBox(shlCmpp,SWT.OK | SWT.ICON_ERROR | SWT.APPLICATION_MODAL);
			box.setText("错误");
			box.setMessage("请输入客户端账号");
			box.open();
			this.textClientAccount.setFocus();
			return false;
		}
		this.textClientAccount.setText(StringUtils.trim(clientAccount));
		
		String clientPassword=this.textClientPassword.getText();
		if(StringUtils.isBlank(clientPassword)) {
			MessageBox box=new  MessageBox(shlCmpp,SWT.OK | SWT.ICON_ERROR | SWT.APPLICATION_MODAL);
			box.setText("错误");
			box.setMessage("请输入客户端账号密码");
			box.open();
			this.textClientPassword.setFocus();
			return false;
		}
		this.textClientPassword.setText(StringUtils.trim(clientPassword));
		
		
		String clientHost=this.textClientHost.getText();
		if(StringUtils.isBlank(clientHost) || !TextVerifyUtils.isboolIp(clientHost)) {
			MessageBox box=new  MessageBox(shlCmpp,SWT.OK | SWT.ICON_ERROR | SWT.APPLICATION_MODAL);
			box.setText("错误");
			box.setMessage("请输入客户端地址");
			box.open();
			this.textClientHost.setFocus();
			return false;
		}
		this.textClientHost.setText(StringUtils.trim(clientHost));
		
		String clientPort=this.textClientPort.getText();
		if(StringUtils.isBlank(clientPort)) {
			MessageBox box=new  MessageBox(shlCmpp,SWT.OK | SWT.ICON_ERROR | SWT.APPLICATION_MODAL);
			box.setText("错误");
			box.setMessage("请输入客户端端口");
			box.open();
			this.textClientPort.setFocus();
			return false;
		}
		this.textClientPort.setText(StringUtils.trim(clientPort));
