package com.ch.sms.simulator.ui.lib;
...
public class SearchText extends StyledText implements PaintListener{
		...
		if(end!=null){
			this.addMouseMoveListener(new MouseMoveListener(){
				...
				public void mouseMove(MouseEvent e) {
					if(SearchText.this.getSize().x-e.x<end.getImageData().width+12){
						SearchText.this.setCursor(new Cursor(Display.getDefault(),SWT.CURSOR_HAND));
					}else{
						SearchText	.this.setCursor(new Cursor(Display.getDefault(),SWT.CURSOR_IBEAM));
					}
	...
	public void addDeleteListener(DeleteListener listener){
		...
		this.addMouseListener(new MouseAdapter(){
			...
			public void mouseUp(MouseEvent e) {
				if(SearchText.this.getCursor().equals(new Cursor(Display.getDefault(),SWT.CURSOR_HAND))){
					SearchText.this.listener.deleteClicked();
				}
			}
